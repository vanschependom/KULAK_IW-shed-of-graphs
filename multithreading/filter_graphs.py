...
def process_graphs(graphs, filters, path, thread):
    ...
    for graph in graphs:
        ...
        if graph:
            ...
            if passed_filters(G, filters):
                ...
                export_graph_image(G, i, path)
    if i == 0:
        print(f"thread_{thread}: No graphs passed the filters.")
        return None
    elif i == 1:
        print(f"thread_{thread}: 1 graph passed the filters.")
    else:
        print(f"thread_{thread}: {i} graphs passed the filters.")
    return passed
...
if __name__ == "__main__":
    ...
    try:
        ...
        raise Exception("No filter was given. Please provide a filter.")
    ...
    # Make the output directory for the thread
    path = f"{output_dir}/output_{thread}"
    ...
    passedGraphs = process_graphs(graphs, filters, path, thread)

    # Write history to history.txt
    write_history(generate_history(len(graphs), passedGraphs, filters), thread)
